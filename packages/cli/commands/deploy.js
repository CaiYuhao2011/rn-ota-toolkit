...
async function deployCommand(options) {
  ...
  for (const targetPlatform of platforms) {
    ...
    try {
      ...
      // 2. 上传文件并发布版本
      const spinner = ora('上传文件并发布版本...').start();

      const formData = new FormData();
      formData.append('bundle', fs.createReadStream(filePath));
      formData.append('appName', app);
      ...
      const response = await axios.post(`${server}/ota/upload`, formData, {
